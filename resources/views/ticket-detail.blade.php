...
<body class="min-h-screen flex flex-col bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-50 transition-colors duration-300" 
    ...
    <header class="border-b border-zinc-200 dark:border-zinc-800 bg-white/50 dark:bg-zinc-950/50 backdrop-blur-md sticky top-0 z-50 transition-colors">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
            ...
            <!-- User Information & Theme Switcher & Logout / Login -->
            <div class="flex items-center gap-4">
                ...
                @else
                    @if (auth()->user()->hasRole('admin'))
                        <a href="/users" class="text-xs font-semibold text-zinc-650 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-200 border border-zinc-200 dark:border-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-700 bg-white/50 dark:bg-zinc-900/40 px-3.5 py-2 rounded-xl transition-all mr-1 shadow-sm">
                            Manage Users
                        </a>
                    @endif
                    <div class="hidden sm:flex flex-col items-end text-right">
                        <span class="text-sm font-semibold text-zinc-900 dark:text-zinc-100">{{ auth()->user()->name }}</span>
                        <span class="text-xs text-zinc-500 dark:text-zinc-400 font-medium capitalize">{{ str_replace('_', ' ', auth()->user()->role) }}</span>
                    </div>
                    <form action="{{ route('logout') }}" method="POST" class="inline">
                        @csrf
                        <button type="submit" class="bg-white hover:bg-zinc-55 dark:bg-zinc-800 dark:hover:bg-zinc-700 active:scale-95 text-xs font-semibold text-zinc-700 dark:text-zinc-200 px-3.5 py-2 rounded-xl transition-all border border-zinc-200 dark:border-zinc-700/50 cursor-pointer shadow-sm">
                            Logout
                        </button>
                    </form>
                @endif
    ...
    <main class="flex-1 mx-auto max-w-7xl w-full px-4 sm:px-6 lg:px-8 py-8 flex flex-col gap-6">
        ...
        <!-- Ticket Header Card -->
        <section class="relative overflow-hidden rounded-2xl border border-zinc-200 dark:border-zinc-800/80 bg-white dark:bg-zinc-900/40 backdrop-blur-xl p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-md dark:shadow-2xl transition-colors">
            <div class="absolute inset-0 bg-gradient-to-tr from-violet-500/5 to-transparent pointer-events-none"></div>
            <div>
                <div class="flex items-center gap-3 mb-2">
                    <span class="px-2.5 py-0.5 rounded-full text-xs font-semibold tracking-wide bg-violet-100 dark:bg-violet-500/10 text-violet-700 dark:text-violet-400 border border-violet-200 dark:border-violet-500/20 uppercase">
                        Ticket Detail
                    </span>
            ...
            <div class="flex flex-col sm:items-end gap-3 shrink-0">
                <div class="text-right">
                    <span class="text-xs text-zinc-450 dark:text-zinc-500 uppercase tracking-widest font-semibold block mb-1">Current State</span>
                    <span :class="{
                        ...
                        'bg-violet-50 dark:bg-violet-500/10 text-violet-750 dark:text-violet-400 border-violet-200 dark:border-violet-500/20': currentStatus === 'done',
                        'bg-red-50 dark:bg-red-500/10 text-red-750 dark:text-red-400 border-red-200 dark:border-red-500/20': currentStatus === 'cancelled'
                    ...
                    <div class="flex flex-wrap gap-2 justify-end">
                        @can('update', $ticket)
                            <button x-show="currentStatus !== 'done' && currentStatus !== 'cancelled'"
                                    ...
                                    class="bg-white hover:bg-zinc-100 dark:bg-zinc-800 dark:hover:bg-zinc-700 border border-zinc-200 dark:border-zinc-700 active:scale-95 transition-all text-xs font-semibold px-4 py-2 rounded-lg text-zinc-750 dark:text-white shadow-sm cursor-pointer">
                                Edit Details
                            </button>
                        @endcan
                        @can('cancel', $ticket)
                        ...
                        <button x-show="currentStatus === 'waiting_destination' && (currentRole === 'dest_manager' || currentRole === 'admin')"
                                ...
                                class="bg-cyan-600 hover:bg-cyan-550 active:scale-95 transition-all text-xs font-semibold px-4 py-2 rounded-lg text-white shadow-md cursor-pointer">
                            Approve Destination
                        </button>
                        ...
                        <button x-show="currentStatus === 'approved_destination' && currentRole === 'admin'"
                                ...
                                class="bg-emerald-600 hover:bg-emerald-550 active:scale-95 transition-all text-xs font-semibold px-4 py-2 rounded-lg text-white shadow-md cursor-pointer">
                            Approve Admin
                        </button>
                        ...
                        <button x-show="currentStatus === 'approved_admin' && currentRole === 'admin'"
                                ...
                                class="bg-amber-600 hover:bg-amber-550 active:scale-95 transition-all text-xs font-semibold px-4 py-2 rounded-lg text-white shadow-md cursor-pointer">
                            Send Cable
                        </button>
                        ...
                        <button x-show="currentStatus === 'sended_cable' && currentRole === 'admin'"
                                ...
                                class="bg-orange-600 hover:bg-orange-550 active:scale-95 transition-all text-xs font-semibold px-4 py-2 rounded-lg text-white shadow-md cursor-pointer">
                            Receive Cable
                        </button>
                        <!-- If Received Cable -> Mark Done -->
                        <button x-show="currentStatus === 'received_cable' && currentRole === 'admin'"
                                @click="transitionStatus('done')"
                                class="bg-violet-600 hover:bg-violet-550 active:scale-95 transition-all text-xs font-semibold px-4 py-2 rounded-lg text-white shadow-md cursor-pointer">
                            Mark Complete (Done)
                        </button>
                    </div>
                @endif
        ...
        <section class="rounded-2xl border border-zinc-200 dark:border-zinc-800/80 bg-white dark:bg-zinc-900/40 backdrop-blur-xl p-6 md:p-8 shadow-sm dark:shadow-xl overflow-hidden transition-colors">
            <h2 class="text-xl font-bold mb-8 flex items-center gap-2 text-zinc-900 dark:text-white">
                    ...
                    <div class="relative z-10 flex flex-row md:flex-col items-center md:items-center gap-4 md:gap-0 md:w-1/6 group">
                        ...
                        <div class="relative flex items-center justify-center shrink-0">
                            ...
                            <div x-show="isActive(stage.key)" 
                                 class="absolute w-12 h-12 rounded-full bg-zinc-200 dark:bg-zinc-850 animate-ping opacity-35"></div>
                            
                            <!-- Circle Status Dot -->
                            <div :class="{
                                     'border-zinc-250 dark:border-zinc-800 bg-zinc-100 dark:bg-zinc-900 text-zinc-400 dark:text-zinc-600': !isCompleted(stage.key),
                                     'border-violet-500 bg-white dark:bg-zinc-950 text-violet-600 dark:text-violet-400 shadow-[0_0_15px_rgba(139,92,246,0.15)] dark:shadow-[0_0_15px_rgba(139,92,246,0.3)]': isActive(stage.key),
                        ...
                        <div class="flex-1 md:text-center mt-0 md:mt-4">
                            <h3 :class="isCompleted(stage.key) ? 'text-zinc-800 dark:text-zinc-100 font-semibold' : 'text-zinc-400 dark:text-zinc-550 font-medium'"
                            ...
                            <div class="mt-1 flex flex-col md:items-center text-xs">
                                <template x-if="getExecutor(stage.key)">
                                    <div class="space-y-0.5">
                                        ...
                                        <span class="text-zinc-450 dark:text-zinc-500 text-[10px]" x-text="getExecutor(stage.key).date + ' @ ' + getExecutor(stage.key).time"></span>
                                    </div>
                                </template>
                                <template x-if="!getExecutor(stage.key)">
                                    <span class="text-zinc-400 dark:text-zinc-600">Pending...</span>
                                </template>
                            </div>
                        </div>
                    </div>
        ...
        <section class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <!-- Source Device Card -->
            <div class="rounded-2xl border border-zinc-200 dark:border-zinc-800/80 bg-white dark:bg-zinc-900/30 backdrop-blur-xl p-6 shadow-md dark:shadow-lg flex flex-col gap-4 transition-colors">
                ...
                @if ($isPublic)
                    <div class="flex-1 flex flex-col items-center justify-center py-6 text-center relative overflow-hidden">
                        ...
                        <div class="absolute inset-0 flex flex-col items-center justify-center p-4 bg-zinc-50/40 dark:bg-zinc-950/20 backdrop-blur-[2px] rounded-xl">
                            <div class="w-8 h-8 rounded-full bg-zinc-100 dark:bg-zinc-850 border border-zinc-200 dark:border-zinc-800 flex items-center justify-center text-zinc-500 dark:text-zinc-400 mb-1.5 shadow-inner">
                ...
                @else
                    <div class="space-y-4">
                        <div>
                            <label class="text-xs text-zinc-450 dark:text-zinc-500 font-semibold block uppercase">Device Name</label>
                            <span class="text-sm text-zinc-800 dark:text-zinc-200 font-medium">{{ $ticket->source_device }}</span>
                        </div>
                        <div>
                            <label class="text-xs text-zinc-455 dark:text-zinc-500 font-semibold block uppercase mb-1">Tenant ID</label>
                            <span class="text-xs font-mono bg-zinc-50 dark:bg-zinc-950/70 border border-zinc-200 dark:border-zinc-800/80 px-2 py-1.5 rounded select-all text-zinc-750 dark:text-zinc-300 block overflow-ellipsis truncate" title="{{ $ticket->source_tenant_id }}">
            ...
            <div class="rounded-2xl border border-zinc-200 dark:border-zinc-800/80 bg-white dark:bg-zinc-900/30 backdrop-blur-xl p-6 shadow-md dark:shadow-lg flex flex-col gap-4 transition-colors">
                ...
                @else
                    <div class="space-y-4">
                        ...
                        <div>
                            <label class="text-xs text-zinc-455 dark:text-zinc-500 font-semibold block uppercase mb-1">Tenant ID</label>
                            <span class="text-xs font-mono bg-zinc-50 dark:bg-zinc-950/70 border border-zinc-200 dark:border-zinc-800/80 px-2 py-1.5 rounded select-all text-zinc-750 dark:text-zinc-300 block overflow-ellipsis truncate" title="{{ $ticket->destination_tenant_id }}">
            ...
            <div class="rounded-2xl border border-zinc-200 dark:border-zinc-800/80 bg-white dark:bg-zinc-900/30 backdrop-blur-xl p-6 shadow-md dark:shadow-lg flex flex-col gap-4 transition-colors">
                <div class="flex items-center gap-3 pb-3 border-b border-zinc-150 dark:border-zinc-800">
                    <div class="w-8 h-8 rounded-lg bg-violet-500/10 text-violet-500 dark:text-violet-400 flex items-center justify-center">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                ...
                @else
                    <div class="space-y-4 flex-1">
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="text-xs text-zinc-450 dark:text-zinc-500 font-semibold block uppercase">Connector Type</label>
                                <span class="text-sm font-semibold text-zinc-800 dark:text-zinc-100">{{ $ticket->connector_type }}</span>
                            </div>
                            <div>
                                <label class="text-xs text-zinc-450 dark:text-zinc-500 font-semibold block uppercase">Deployment Mode</label>
                        ...
                        <div class="bg-zinc-50 dark:bg-zinc-950/50 rounded-xl p-3 border border-zinc-200 dark:border-zinc-800 transition-colors">
                            <label class="text-xs text-zinc-450 dark:text-zinc-500 font-semibold block uppercase mb-1">JSON Metadata Details</label>
        ...
        <section class="rounded-2xl border border-zinc-200 dark:border-zinc-800/80 bg-white dark:bg-zinc-900/40 backdrop-blur-xl p-6 md:p-8 shadow-sm dark:shadow-xl min-h-[220px] flex flex-col transition-colors">
            ...
            @if ($isPublic)
                <div class="flex-1 flex flex-col items-center justify-center py-8 text-center relative overflow-hidden">
                    ...
                    <!-- Lock Centered Overlay -->
                    <div class="absolute inset-0 flex flex-col items-center justify-center p-4 bg-zinc-50/40 dark:bg-zinc-950/20 backdrop-blur-[2px] rounded-xl">
                        <div class="w-10 h-10 rounded-full bg-zinc-100 dark:bg-zinc-850 border border-zinc-200 dark:border-zinc-800 flex items-center justify-center text-zinc-550 dark:text-zinc-400 mb-2 shadow-inner">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 1 0-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 0 0 2.25-2.25v-6.75a2.25 2.25 0 0 0-2.25-2.25H6.75a2.25 2.25 0 0 0-2.25 2.25v6.75a2.25 2.25 0 0 0 2.25 2.25Z" />
                            </svg>
                        </div>
            ...
            @else
                ...
                <div class="overflow-x-auto hidden md:block">
                    <table class="w-full border-collapse text-left">
                        <thead>
                            <tr class="border-b border-zinc-200 dark:border-zinc-800 text-xs font-bold text-zinc-500 dark:text-zinc-450 uppercase tracking-wider">
                        ...
                        <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800/60 text-sm text-zinc-700 dark:text-zinc-300">
                            ...
                            <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/25 transition-colors duration-200">
                                ...
                                <td class="py-3 px-4">
                                    <span class="px-2 py-0.5 rounded text-[10px] font-bold border capitalize" 
                                          :class="{
                                              'bg-violet-100 dark:bg-violet-500/10 text-violet-750 dark:text-violet-400 border-violet-200 dark:border-violet-500/20': '{{ $log->user->role ?? '' }}' === 'admin',
                                              'bg-cyan-100 dark:bg-cyan-500/10 text-cyan-750 dark:text-cyan-400 border-cyan-200 dark:border-cyan-500/20': '{{ $log->user->role ?? '' }}' === 'dest_manager',
                                ...
                                <td class="py-3 px-4 text-right text-zinc-450 dark:text-zinc-500">
                ...
                <div class="block md:hidden space-y-4">
                    @forelse($ticket->logs as $log)
                        <div class="p-4 rounded-xl border border-zinc-250 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/20 flex flex-col gap-2.5 transition-colors">
                            <div class="flex items-center justify-between">
                                ...
                                <span class="text-[11px] text-zinc-450 dark:text-zinc-550 font-medium">
                            ...
                            <div class="text-[10px] text-zinc-450 dark:text-zinc-500 text-right">
    ...
    <div x-show="showEditModal" 
        ...
        <div class="relative w-full max-w-lg bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 shadow-2xl flex flex-col gap-6"
            ...
            <div class="flex items-center justify-between pb-3 border-b border-zinc-200 dark:border-zinc-800">
                ...
                <button @click="showEditModal = false" class="text-zinc-400 hover:text-zinc-600 dark:text-zinc-550 dark:hover:text-zinc-300 cursor-pointer">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
            ...
            <form @submit.prevent="updateTicket" class="space-y-4">
                <div>
                    <label class="text-xs text-zinc-650 dark:text-zinc-400 font-semibold block mb-1">Ticket Label (Unique)</label>
                    <input type="text" x-model="editTicket.label" required placeholder="e.g. TICKET-101"
                           class="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-250 dark:border-zinc-800 rounded-lg px-3 py-2 text-sm text-zinc-900 dark:text-white focus:outline-none focus:border-violet-650 dark:focus:border-violet-600 transition-colors">
                </div>

                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="text-xs text-zinc-650 dark:text-zinc-400 font-semibold block mb-1">Source Device</label>
                        <input type="text" x-model="editTicket.source_device" required placeholder="e.g. JKT-SW-01"
                               class="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-250 dark:border-zinc-800 rounded-lg px-3 py-2 text-sm text-zinc-900 dark:text-white focus:outline-none focus:border-violet-650 dark:focus:border-violet-600 transition-colors">
                    </div>
                    <div>
                        <label class="text-xs text-zinc-655 dark:text-zinc-400 font-semibold block mb-1">Destination Device</label>
                        <input type="text" x-model="editTicket.destination_device" required placeholder="e.g. SG-SW-02"
                               class="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-250 dark:border-zinc-800 rounded-lg px-3 py-2 text-sm text-zinc-900 dark:text-white focus:outline-none focus:border-violet-655 dark:focus:border-violet-600 transition-colors">
                    </div>
                </div>

                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="text-xs text-zinc-650 dark:text-zinc-400 font-semibold block mb-1">Connector Type</label>
                    ...
                    <div>
                        <label class="text-xs text-zinc-650 dark:text-zinc-400 font-semibold block mb-1">Cable Length (m)</label>
                ...
                <div>
                    <label class="text-xs text-zinc-650 dark:text-zinc-400 font-semibold block mb-1">Cable Color</label>
                ...
                <div class="flex justify-end gap-3 pt-4 border-t border-zinc-200 dark:border-zinc-800">
                    ...
                    <button type="submit"
                            class="bg-violet-600 hover:bg-violet-550 text-xs font-semibold px-4 py-2.5 rounded-lg text-white transition-colors cursor-pointer">
                        Save Changes
                    </button>
                </div>
            </form>
    ...
    <footer class="border-t border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 py-6 text-center text-xs text-zinc-500 dark:text-zinc-600 transition-colors">
        &copy; 2026 General Ticketing System. Built with Premium ProMax UI.
    </footer>
</body>
</html>
